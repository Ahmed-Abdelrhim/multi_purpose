...
<body
    @if (session()->has('language_direction_from_dropdown')) @if(session()->get('language_direction_from_dropdown') == 1)  class="rtl-version"
    @endif @elseif (isset($language)) @if ($language->direction == 1) class="rtl-version" @endif  @endif >
<!-- Preloader -->
...
<div class="main-container-wrapper">
        ...
        <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center">
            @if (isset($general_site_image))
                ...
                @if (!empty($general_site_image->admin_logo_image))
                    <a class="navbar-brand brand-logo mr-5" href="{{ url('dashboard') }}">
                        <img src="{{ asset('uploads/img/general/'.$general_site_image->admin_logo_image) }}"
                             class="mr-2" alt="logo"/>
                    </a>
                @endif
                ...
                @if (!empty($general_site_image->admin_small_logo_image))
                    <a class="navbar-brand brand-logo-mini" href="{{ url('dashboard') }}">
                        <img src="{{ asset('uploads/img/general/'.$general_site_image->admin_small_logo_image) }}"
                             alt="logo"/>
                    </a>
                @endif
            ...
            @if (isset($general_creative_site_image))
                ...
                @if (!empty($general_creative_site_image->admin_logo_image))
                    <a class="navbar-brand brand-logo mr-5" href="{{ url('dashboard') }}">
                        <img
                            src="{{ asset('uploads/creative/img/general/'.$general_creative_site_image->admin_logo_image) }}"
                            class="mr-2" alt="logo"/>
                    </a>
                @endif
                ...
                @if (!empty($general_creative_site_image->admin_small_logo_image))
                    <a class="navbar-brand brand-logo-mini" href="{{ url('dashboard') }}">
                        <img
                            src="{{ asset('uploads/creative/img/general/'.$general_creative_site_image->admin_small_logo_image) }}"
                            alt="logo"/>
                    </a>
                @endif
        ...
        <div class="navbar-menu-wrapper d-flex align-items-center justify-content-between">
            ...
            <ul class="navbar-nav mr-lg-2">
                <li class="nav-item d-none d-md-block">
                    ...
                    {{ __('content.data_language') }}: {{ $data_language->language_name }} <i
                            class="fas fa-hand-point-right ml-1"></i>
                    ...
                    <li class="nav-item dropdown dropdown-animate">
                        <a class="nav-link count-indicator dropdown-toggle" id="notificationDropdown" href="#"
                           data-toggle="dropdown">
                            ...
                            @endif<i class="arrow_carrot-down"></i>
                        </a>
                        <div class="dropdown-menu dropdown-menu-right navbar-dropdown preview-list"
                            ...
                            @foreach ($display_dropdowns as $display_dropdown)
                                <a href="{{ url('language/set-locale/'.$display_dropdown->id) }}"
                                   class="dropdown-item preview-item d-flex align-items-center">{{ $display_dropdown->language_name }}</a>
                            @endforeach
                ...
                <li class="nav-item nav-profile dropdown dropdown-animate">
                    <a class="nav-link dropdown-toggle" href="#" data-toggle="dropdown" id="profileDropdown">
                        @if(Auth::user()->profile_photo_path != null)
                            <img src="{{ asset('uploads/img/profile/'.Auth::user()->profile_photo_path) }}"
                                 class="img-profile rounded-circle" alt="profile image">
                        @else
                            <img src="{{ asset('uploads/img/dummy/128x128.jpg') }}" class="img-profile rounded-circle"
                                 alt="profile image">
                        @endif
                    </a>
                    <div class="dropdown-menu dropdown-menu-right navbar-dropdown profile-top"
                         aria-labelledby="profileDropdown">
                        <a href="{{ url('admin/profile/edit') }}" class="dropdown-item"><i
                                class="fas fa-user profile-icon" aria-hidden="true"></i> {{ __('content.profile') }}</a>
                        <a href="{{ url('admin/profile/change-password') }}" class="dropdown-item"><i
                                class="fas fa-unlock-alt profile-icon"
                                aria-hidden="true"></i> {{ __('content.change_password') }}</a>
        ...
        <nav class="sidebar sidebar-offcanvas" id="sidebar">
            <ul class="nav">
                ...
                <li class="nav-item {{ (request()->is('admin/admin-role') ||
                    ...
                    <a class="nav-link" data-toggle="collapse" href="#admin_roles" aria-expanded="false"
                       aria-controls="admin_roles">
                    ...
                    <div class="collapse {{ (request()->is('admin/admin-role') ||
                        ...
                        <ul class="nav flex-column sub-menu">
                            <li class="nav-item"><a
                                    class="nav-link {{ (request()->is('admin/admin-role/create')) ? 'active' : '' }}"
                                    href="{{ url('admin/admin-role/create') }}">{{ __('content.add_admin_role') }}</a>
                            </li>
                            <li class="nav-item"><a
                                    class="nav-link {{ (request()->is('admin/admin-role')) ? 'active' : '' }}"
                                    href="{{ url('admin/admin-role') }}">{{ __('content.admin_roles') }}</a></li>
                        </ul>
                    </div>
                </li>
                <li class="nav-item {{ (request()->is('admin/admin-user') ||
                    ...
                    <a class="nav-link" data-toggle="collapse" href="#admins" aria-expanded="false"
                    ...
                    <div class="collapse {{ (request()->is('admin/admin-user') ||
                        ...
                        <ul class="nav flex-column sub-menu">
                            <li class="nav-item"><a
                                    class="nav-link {{ (request()->is('admin/admin-user/create')) ? 'active' : '' }}"
                                    href="{{ url('admin/admin-user/create') }}">{{ __('content.add_admin_user') }}</a>
                            </li>
                            <li class="nav-item"><a
                                    class="nav-link {{ (request()->is('admin/admin-user')) ? 'active' : '' }}"
                                    href="{{ url('admin/admin-user') }}">{{ __('content.all_admin') }}</a></li>
                        </ul>
                    </div>
                </li>
                @endhasrole
                ...
                @can('subscribe check')
                    <li class="nav-item {{ (request()->is('admin/subscribe/create')) ? 'active' : '' }}">
                        ...
                        <div class="collapse {{ (request()->is('admin/subscribe/create')) ? 'show' : '' }}"
                             id="subscribers">
                            <ul class="nav flex-column sub-menu">
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/subscribe/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/subscribe/create') }}">{{ __('content.subscribers') }}</a>
                                </li>
                            </ul>
                        </div>
                    </li>
                ...
                @can('banner check')
                    <li class="nav-item {{ (request()->is('admin/fixed-content/create') ||
                        ...
                        <a class="nav-link" data-toggle="collapse" href="#advanced" aria-expanded="false"
                        ...
                        <div class="collapse {{ (request()->is('admin/fixed-content/create') ||
                                                 ...
                                                 request()->is('admin/slider/*/edit') ||
                                                 request()->is('admin/video/create') ||
                                            request()->is('admin/homepage-version/create')) ? 'show' : '' }}"
                             id="advanced">
                            <ul class="nav flex-column sub-menu">
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/fixed-content/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/fixed-content/create') }}">{{ __('content.fixed_content') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/slider/create') || request()->is('admin/slider/*/edit')) ? 'active' : '' }}"
                                        href="{{ url('admin/slider/create') }}">{{ __('content.sliders') }}</a></li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/video/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/video/create') }}">{{ __('content.video') }}</a></li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/homepage-version/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/homepage-version/create') }}">{{ __('content.homepage_versions') }}</a>
                                </li>
                            </ul>
                        </div>
                    </li>
                ...
                @can('blogs check')
                    <li class="nav-item {{ (request()->is('admin/blog') ||
                        ...
                        <div class="collapse {{ (request()->is('admin/blog') ||
                                                  ...
                                                  request()->is('admin/category/create') ||
                                                  request()->is('admin/category/*/edit') ||
                                                  request()->is('admin/blog-paginate/create')) ? 'show' : '' }}"
                             id="blogs">
                            <ul class="nav flex-column sub-menu">
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/category/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/category/create') }}">{{ __('content.categories') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/blog/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/blog/create') }}">{{ __('content.add_blog') }}</a></li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/blog')) ? 'active' : '' }}"
                                        href="{{ url('admin/blog') }}">{{ __('content.blogs') }}</a></li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/blog-paginate/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/blog-paginate/create') }}">{{ __('content.blog_paginate') }}</a>
                                </li>
                            </ul>
                        </div>
                    </li>
                ...
                @can('services check')
                    <li class="nav-item {{ (request()->is('admin/service') ||
                        ...
                        <div class="collapse {{ (request()->is('admin/service') ||
                                            ...
                                            request()->is('admin/service-detail/*/*/edit') ||
                                                  request()->is('admin/service-paginate/create') ||
                                                  request()->is('admin/service-background-image/create')) ? 'show' : '' }}"
                             id="services">
                            <ul class="nav flex-column sub-menu">
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/service/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/service/create') }}">{{ __('content.add_service') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/service')) ? 'active' : '' }}"
                                        href="{{ url('admin/service') }}">{{ __('content.services') }}</a></li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/service-paginate/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/service-paginate/create') }}">{{ __('content.service_paginate') }}</a>
                                </li>
                            </ul>
                        </div>
                    </li>
                ...
                @can('portfolio check')
                    <li class="nav-item {{ (request()->is('admin/portfolio') ||
                        ...
                        <div class="collapse {{ (request()->is('admin/portfolio') ||
                                                  ...
                                                  request()->is('admin/portfolio-slider/*/*/edit') ||
                                                  request()->is('admin/portfolio-detail/*/create') ||
                                                  request()->is('admin/portfolio-detail/*/*/edit')) ? 'show' : '' }}"
                             id="portfolios">
                            <ul class="nav flex-column sub-menu">
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/portfolio-category/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/portfolio-category/create') }}">{{ __('content.categories') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/portfolio/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/portfolio/create') }}">{{ __('content.add_portfolio') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/portfolio')) ? 'active' : '' }}"
                                        href="{{ url('admin/portfolio') }}">{{ __('content.portfolios') }}</a></li>
                            </ul>
                        </div>
                    </li>
                @endcan
                        ...
                        <div class="collapse {{ (request()->is('admin/contact/create') ||
                            ...
                            <ul class="nav flex-column sub-menu">
                                <li class="nav-item"><a class="nav-link {{ (request()->is('admin/contact/create') ||
                                                                             request()->is('admin/contact/*/edit')) ? 'active' : '' }}"
                                                        href="{{ url('admin/contact/create') }}">{{ __('content.contact_info') }}</a>
                                </li>
                                <li class="nav-item"><a class="nav-link {{ (request()->is('admin/social/create') ||
                                                                             request()->is('admin/social/*/edit')) ? 'active' : '' }}"
                                                        href="{{ url('admin/social/create') }}">{{ __('content.socials') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/quick-access/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/quick-access/create') }}">{{ __('content.quick_access_buttons') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/message')) ? 'active' : '' }}"
                                        href="{{ url('admin/message') }}">{{ __('content.messages') }}</a></li>
                            </ul>
                        </div>
                    </li>
                @endcan
                @can('settings check')
                    <li class="nav-item {{ (request()->is('admin/site-info/create') ||
                        ...
                        <div class="collapse {{ (request()->is('admin/site-info/create') ||
                            ...
                            <ul class="nav flex-column sub-menu">
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/site-info/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/site-info/create') }}">{{ __('content.site_info') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/site-image/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/site-image/create') }}">{{ __('content.site_images') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/google-analytic/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/google-analytic/create') }}">{{ __('content.google_analytic') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/breadcrumb/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/breadcrumb/create') }}">{{ __('content.breadcrumb') }}</a>
                                </li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/section/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/section/create') }}">{{ __('content.sections') }}</a></li>
                                <li class="nav-item"><a
                                        class="nav-link {{ (request()->is('admin/color-option/create')) ? 'active' : '' }}"
                                        href="{{ url('admin/color-option/create') }}">{{ __('content.color_option') }}</a>
                                </li>
                                <li class="nav-item"><a
    ...
    <div class="modal fade" id="processedLanguageModal" tabindex="-1" role="dialog"
         aria-labelledby="processedLanguageModalLabel" aria-modal="false">
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0 font-16"
                        id="processedLanguageModalLabel">{{ __('content.which_language') }}</h5>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="language_id" id="language_id" required>
                                        @foreach ($languages as $lang)
                                            <option
                                                value="{{ $lang->id }}" {{ $lang->status == 1 ? 'selected' : '' }}>{{ $lang->language_name }}</option>
                                        @endforeach
                                        @php unset($lang); @endphp
                                    </select>
                                    <small id="language_id"
                                           class="form-text text-muted">{{ __('content.reminding') }}</small>
                                </div>
                            </div>
                            <div class="col-12">
